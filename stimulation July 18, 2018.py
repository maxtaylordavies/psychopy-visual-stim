...
def stimulation(SpatFreqDeg, TempFreq, t_before, T_During, t_after, WinWidth, DispHeight, win, Synch, Motionmode):
    ...
    if Synch:
        ...
        try:
            #waiting for the signal autoVs or manualVS
        ...
        print("Length of UDP communation time: " + str((psychopy.core.getTime() - x)/4.0) + " seconds.")
    ...
    if controlmod == 'maualVS':
        ...
        for m in range(parasize):
            ...
            if m == 0: 
                ...
                pixelformeanlum = (np.exp(np.log(MeanLum/AmpFactor)/GammaFactor)/255.0)*2 -1
                gray = MeanLum
                inc = gray*contrast
    ...
    if Synch == False:
        ...
        #looping the stimulus so that it continues to run
        while True:
