def movSinGrat(MovSinGrat_SpatFreqVal, MovSinGrat_tempFreqVal, MovSinGrat_t_before, MovSinGrat_t_During, MovSinGrat_t_after, Synch, MovSinGrat_Motionmode, MovSinGrat_features, MovSinGrat_ledstate, MovSinGrat_Rep):
    ''' 
    ...
    MovSinGrat_t_triginit, GammaFactor, AmpFactor, MovSinGrat_contrast, MovSinGrat_MeanLum, win, MovSinGrat_spat_list,
    winWidth , winHeight, ScrnNum, PixelSize, winWidthofEachDisp, DisplayFrameWidth, FR, square1, square2, MovSinGrat_one_screen, mask_L, mask_R, fontSize, fontClr, win, Local_IP, Local_Port, Remote_IP, Remote_Port, ani_distance,
    MovSinGrat_randomseq, MovSinGrat_features_dict, MovSinGrat_angles_list, MovSinGrat_temp_lin_list, MovSinGrat_temp_osc_list, MovSinGrat_location_list, MovSinGrat_contrast_list)
    ...
    if Synch:
        ...
        vs_path='Y:/ephys/VSpara/'
        eph_path='Y:/ephys/ephys_data/'
        ...
        fileName = vs_path + eph_explist[latesttimeindex] + '/vsparam'    #exp name defined above either by user (if not synch) or by eye tracking software (if user)
        ...
        if MovSinGrat_features == 0: #ori
            ...
            tuning_stim_val = map(float, MovSinGrat_angles_list) #map applies the float() function to all elements of the list, therefore converting strings to float
        ...
        for repind in xrange(tot_num_stim): 
            ...
            print(paras)74444444444444444444444444444444    
            #adding the parameters as an array at index i
            for localstimid in xrange(0,(len(stimId))):
            
                if MovSinGrat_features == 0: #0 = ori
                    paras[repind*(len(stimId))+localstimid, :] = [MovSinGrat_SpatFreqVal, MovSinGrat_tempFreqVal, MovSinGrat_contrast, MovSinGrat_MeanLum, MovSinGrat_dirindex, MovSinGrat_t_before, MovSinGrat_t_During, MovSinGrat_t_after, MovSinGrat_t_triginit, tuning_stim_val[stimId[localstimid]], MovSinGrat_ledstate, 0] # ADD LOCATION 
    ...
    pixelangle = np.empty(shape=[1, fix_spatfreq*winWidth]) #pixel has to be 2D since the image is 2D
    ...
    for i in range(ScrnNum):
        pixelangle[:,i*fix_spatfreq*winWidthofEachDisp: (i + 1)*fix_spatfreq*winWidthofEachDisp ] = tempPixelAngle + 90*i #taking specific ranges within the full winWidth and replacing the values with the corresponding angles
    
    exit_flag = 0
    
    #Genereating the VS based on the parameters in paras 
    for m in xrange(MovSinGrat_Rep):
        ...
        if m == 0:
            ...
            pixelformeanlum = 2*(np.exp(np.log(MovSinGrat_MeanLum/AmpFactor)/GammaFactor)/255.0) -1
            ...
            if MovSinGrat_Motionmode == 1:
                ...
                phase = MovSinGrat_Amp_sinu*np.sin((phase/frames)*2*np.pi)*SpatFreqDeg*2*np.pi
            ...
            for i in range(int(frames)):
                texdata1DTmp = np.exp(np.log((MovSinGrat_gray + inc*np.sin(pixelangle*SpatFreqDeg*2*np.pi + phase[i]))/AmpFactor)/GammaFactor)
