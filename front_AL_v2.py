...
def front():
    ...
    while keep_going:
        ...
        #For PLANAR 
            ...
            square2.draw()
            win.flip()
            stimulationBv2.stimulation(SpatFreqValB, tempFreqValB, t_beforeValB, t_stimValB, t_afterValB, winWidth, winHeight, win, syncVal, MotionmodeB, Ori)
            keys = event.getKeys() #retrieving key presses from the buffer during the stimulation
            keys = [] #clearing the key presses
            square1.fillColor = [-1,-1,-1]
            square2.fillColor = [-1,-1,-1]
