def movSinGrat(
    ...
    # if Synch is False, this else condition will make the parameters in the same format as if Synch was True
        ...
        instruction_text = visual.TextStim(
            win, text=u"Name experiment and press enter to start.", pos=(0, 0.5)
        )
        ...
        while now:
            ...
            if (
                key
                in "1234567890abcdfeghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-"
            ):
        ...
        paras = np.empty(shape=[tot_num_stim, 17])
